...
<template>
    <div v-if="!hidden" id="control" :class="on ? 'on' : 'off'">
        <div :class="style">
            <div v-if="features.volume && on" class="background">
                <div class="inner"></div>
            </div>
            <svg v-if="features.volume && on" viewBox="0 0 100 100" v-on:click.stop="select" v-on:mousedown.prevent="start(true)" v-on:touchstart.prevent="start()">
                <path class="range" :d="range" />
                <path v-if="visable" class="marker" :d="marker" />
            </svg>
            <div class="switch">
                <div class="inner" v-on:click="toggle">
                    <icon v-if="subject" name="television" class="icon" />
                </div>
            </div>
    ...
    export default {
        ...
        computed: {
            ...
            style() {
                ...
                if (this.on && this.features.volume) result = `${result} volume`;
            ...
            active() {
                return {
                    x: MID_X + Math.cos(this.map(this.volume, 0, 100, MIN_RADIANS, MAX_RADIANS)) * RADIUS,
                    y: MID_Y - Math.sin(this.map(this.volume, 0, 100, MIN_RADIANS, MAX_RADIANS)) * RADIUS,
                };
            },

            arc() {
                return Math.abs(this.map(0, 0, 100, MIN_RADIANS, MAX_RADIANS) - this.map(this.volume, 0, 100, MIN_RADIANS, MAX_RADIANS)) < Math.PI ? 0 : 1;
            },

            sweep() {
                return this.map(this.volume, 0, 100, MIN_RADIANS, MAX_RADIANS) > this.map(0, 0, 100, MIN_RADIANS, MAX_RADIANS) ? 0 : 1;
            },
        },
        ...
        data() {
            return {
                ...
                updater: Debounce(() => {
                    if (!this.local) {
                        const volume = this.subject.characteristics.find((item) => item.type === "volume");
                        ...
                        if (volume) this.features.volume = true;
                ...
                commit: Debounce(async () => {
                    if (!this.disabled && this.on) {
                        ...
                        await accessory.set("volume", Math.round(this.volume));
            ...
            update(offsetX, offsetY) {
                if (!this.disabled && this.on) {
                    ...
                    if (angle > MAX_RADIANS) {
                        this.volume = this.map(angle, MIN_RADIANS, MAX_RADIANS, 0, 100);
                    } else if (angle < start) {
                        this.volume = this.map(angle + 2 * Math.PI, MIN_RADIANS, MAX_RADIANS, 0, 100);
                    } else {
                        return;
                    }

                    this.on = this.volume > 0;
                    this.$emit("input", Math.round(this.volume));
                }
            },
